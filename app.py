        ...
        with st.spinner("scraping upto 500 comments from Youtube..."):
            while True:
                ...
                for item in response.get("items",[]): #accessing one comment at a time 
                    ...
                    if len(comments_list) >= 100: #stops storing comments in the list when we hit a 500 comment limit 
                        break   
                ...
                if not next_page_token or len(comments_list) >= 100: #stops storing comments in the list when we hit a 500 comment limit or no nxt page exists (whichever happens first)
                    break
